...
- **甲骨文拼图**：把甲骨文打乱成 3×3 小块，点两块交换拼回去。
- **查甲骨文字典**：输入一个字 / 词 / 名字看甲骨文，可浏览整个字库；库里没有的字标注「暂无甲骨文」。
...
「查甲骨文字典」用的字形(约 793 个)来自 **oraclebone.org**（作者 Xiaoliang Wang），
授权 **CC BY-NC 4.0（署名-非商业）**——即本网站须为**免费、非商业**用途，并已在板块底部注明出处。
图片在 `assets/oracle/`（多为 `u{码点}.jpg`），索引在 `assets/oracle-index.js`，
抓取脚本见 `tools/fetch_oracle.py`（需挂代理）。字库里没有的字会回退到本项目手绘的那几个。
